...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/28 14:35:38 by tzenyatt          #+#    #+#             */
/*   Updated: 2021/01/28 14:42:15 by tzenyatt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_printf		*fill_in_struct(char **format, va_list argptr)
{
	...
	if (structure != 0)
	{
		if (flag_parcer(structure, format) == 0 ||
				width_parcer(structure, format, argptr) == 0 ||
				parce_precision(structure, format, argptr) == 0)
			can_i_parce_next = 0;
