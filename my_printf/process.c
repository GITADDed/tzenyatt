...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/28 14:47:37 by tzenyatt          #+#    #+#             */
/*   Updated: 2021/01/28 14:53:16 by tzenyatt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	print_process_num(t_printf *structure, long long num, int base, char sign)
{
	...
	if (structure->precision != -1 || structure->minus != -1)
		structure->zero = -1;
	num_len = ((num == 0 &&
				structure->precision == 0) ? 0 : n_length(num, base));
	if (structure->minus != '-')
...
int	print_width(t_printf *structure, int len, char sign)
{
	...
	size = 0;
	if (sign != -1 && structure->zero != -1 &&
			structure->precision == -1 && structure->minus == -1)
		ft_putchar(sign);
	if (structure->width > len && structure->width > structure->precision)
	{
		structure->width -= ((structure->precision > len) ?
...
int	print_precision(t_printf *structure, int len, char sign)
{
	...
	size = 0;
	if (sign != -1 && (structure->zero == -1 ||
				structure->precision != -1 || structure->minus == '-'))
		ft_putchar('-');
	if (structure->type == 'p')
		ft_putstr("0x");
